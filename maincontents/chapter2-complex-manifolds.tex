...
\begin{figure}[h]
  ...
  %%
  \node (b1) at ([xshift=8cm]a1) {$\pi^{-1}(x)$ $\subset \pi^{-1}(U_i)\subset E$};
  \node (b2) at ([xshift=8cm]a2) {$\bC^r$ $ \subset U_i \times \bC^r$};
  \node[right] (b3) at ([xshift=4cm]b1) {$U_i\subset M (=\bigcup U_i)$};
  \draw[->] ([xshift=1.1cm]b1.south west) --node[left] {$\psi_i (x)$} node[right] {$\cong$}([xshift=-1.15cm]b2.north) ;
  \draw[->] ([xshift=-1.78cm]b1.south east) --node[right] {$\psi_i$} node[left] {$\cong$} ([xshift=0.5cm]b2.north) ;
